...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
     ...
     */
    protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException { //[0] is left, [1] is right
        // TODO: Implement this method.
	 if (child == null || parent == null) {
            throw new IllegalArgumentException("Null argument has been passed");
        }

        boolean isRight = child.isRightChild();
        boolean isRoot = (root == parent);

        //check if parent is root, if not replace grandparents down[] to child instead of parent
        if (!isRoot) {
            boolean pIsRight = parent.isRightChild();
            if (pIsRight) {
        ...
        if (!isRight) { // right rotation
            if (child.down[1] != null) {
                //set child's right child to left child of parent
            ...
            child.down[1] = parent;
        } else if (isRight) { // left rotation
            if (child.down[0] != null) {
                //set child's left child to right child of parent
            ...
            throw new IllegalArgumentException("node references not related");
        }
    ...
    public static boolean test1() { //Right Rotation
    	...
    	Node<Integer> twelve = tester.root;
    	Node<Integer> seven = twelve.down[0];
    	
    	tester.rotate(seven, twelve);
    	
if		(!tester.toLevelOrderString().equals("[ 7, 3, 12, 10, 23 ]")) {
    ...
    public static boolean test2() { //Left Rotation
        // TODO: Implement this test.
    	BinarySearchTree<Integer> tester = new BinarySearchTree<Integer>();
    	tester.insert(5);
    	tester.insert(3);
    	...
    	Node<Integer> five = tester.root;
    	Node<Integer> nine = five.down[1];
    	
    	tester.rotate(nine, five);
    	
    	
    	if (!tester.toLevelOrderString().equals("[ 9, 5, 11, 3, 7, 12 ]")) {
    ...
    public static boolean test3() { // double rotation, left-left
        // TODO: Implement this test.
    	...
    	Node<Integer> five = tester.root;
    	Node<Integer> nine = five.down[1];
    	Node<Integer> eleven = nine.down[1];
    	
    	tester.rotate(nine, five);
    	
    	//second rotation
    	tester.rotate(eleven, nine);
    	
    	if (!tester.toLevelOrderString().equals("[ 11, 9, 12, 5, 3, 7 ]")) {
